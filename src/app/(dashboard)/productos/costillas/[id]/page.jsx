...
export default function EditarCostillasPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const product = await productsService.getById('costillas', params.id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast.success('Costillas actualizadas exitosamente');
      ...
      <Card>
        <div className="mb-6">
          <Button
            variant="ghost"
            icon={FaArrowLeft}
            onClick={() => router.push('/productos/costillas')}
          >
            Volver
          </Button>
        </div>

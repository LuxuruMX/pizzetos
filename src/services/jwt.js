...
export const getUsuarioIdFromToken = () => {
  const token = localStorage.getItem('access_token');
  ...
  try {
    ...
    // Busca id_emp primero, luego otros campos numéricos como fallback
    return payload.id_emp || payload.id || payload.user_id;
  } catch (error) {
    console.error('Error decodificando token para ID usuario:', error);
    return null;
  }
};
